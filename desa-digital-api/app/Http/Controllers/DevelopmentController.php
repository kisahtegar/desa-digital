...
class DevelopmentController extends Controller implements HasMiddleware
{
    ...
    {
        return [
            new Middleware(PermissionMiddleware::using(permission: ['development-list|development-create|development-edit|development-delete']), only: ['index', 'getAllPaginated', 'show']),
            new Middleware(PermissionMiddleware::using(permission: ['development-create']), only: ['store']),
            new Middleware(PermissionMiddleware::using(permission: ['development-edit']), only: ['update']),
            new Middleware(PermissionMiddleware::using(permission: ['development-delete']), only: ['destroy']),
        ];
    }
    ...
    public function show(string $id)
    {
        try {
            ...
            return ResponseHelper::jsonResponse(true, 'Detail Pembangunan Berhasil Ditemukan', new DevelopmentResource($development), 200);
        } catch (\Exception $e) {
            return ResponseHelper::jsonResponse(false, $e->getMessage(), null, 500);
        }
    }
