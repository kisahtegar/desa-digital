...
class DevelopmentApplicantController extends Controller implements HasMiddleware
{
    ...
    {
        return [
            new Middleware(PermissionMiddleware::using(permission: ['development-applicant-list|development-applicant-create|development-applicant-edit|development-applicant-delete']), only: ['index', 'getAllPaginated', 'show']),
            new Middleware(PermissionMiddleware::using(permission: ['development-applicant-create']), only: ['store']),
            new Middleware(PermissionMiddleware::using(permission: ['development-applicant-edit']), only: ['update']),
            new Middleware(PermissionMiddleware::using(permission: ['development-applicant-delete']), only: ['destroy']),
        ];
    }
    ...
    public function show(string $id)
    {
        try {
            ...
            return ResponseHelper::jsonResponse(true, 'Detail Pendaftar Pembangunan Berhasil Ditemukan', new DevelopmentApplicantResource($developmentApplicant), 200);
        } catch (\Exception $e) {
            return ResponseHelper::jsonResponse(false, $e->getMessage(), null, 500);
        }
    }
