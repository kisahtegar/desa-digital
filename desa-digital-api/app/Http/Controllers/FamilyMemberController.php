...
class FamilyMemberController extends Controller implements HasMiddleware
{
    ...
    {
        return [
            new Middleware(PermissionMiddleware::using(permission: ['family-member-list|family-member-create|family-member-edit|family-member-delete']), only: ['index', 'getAllPaginated', 'show']),
            new Middleware(PermissionMiddleware::using(permission: ['family-member-create']), only: ['store']),
            new Middleware(PermissionMiddleware::using(permission: ['family-member-edit']), only: ['update']),
            new Middleware(PermissionMiddleware::using(permission: ['family-member-delete']), only: ['destroy']),
        ];
    }
    ...
    public function show(string $id)
    {
        try {
            ...
            return ResponseHelper::jsonResponse(true, 'Detail Anggota Keluarga Berhasil Ditemukan', new FamilyMemberResource($familyMember), 200);
        } catch (\Exception $e) {
            return ResponseHelper::jsonResponse(false, $e->getMessage(), null, 500);
        }
    }
