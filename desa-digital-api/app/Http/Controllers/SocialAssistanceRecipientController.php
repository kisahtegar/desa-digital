...
class SocialAssistanceRecipientController extends Controller implements HasMiddleware
{
    ...
    {
        return [
            new Middleware(PermissionMiddleware::using(permission: ['social-assistance-recipient-list|social-assistance-recipient-create|social-assistance-recipient-edit|social-assistance-recipient-delete']), only: ['index', 'getAllPaginated', 'show']),
            new Middleware(PermissionMiddleware::using(permission: ['social-assistance-recipient-create']), only: ['store']),
            new Middleware(PermissionMiddleware::using(permission: ['social-assistance-recipient-edit']), only: ['update']),
            new Middleware(PermissionMiddleware::using(permission: ['social-assistance-recipient-delete']), only: ['destroy']),
        ];
    }
    ...
    {
        try {
            $socialAssitanceRecipient = $this->socialAssistanceRecipientRepository->getById($id);

            if (!$socialAssitanceRecipient) {
                return ResponseHelper::jsonResponse(false, 'Data Penerima Bantuan Sosial Tidak Ditemukan', null, 404);
            }

            return ResponseHelper::jsonResponse(true, 'Detail Bantuan Sosial Berhasil Diambil', new SocialAssistanceRecipientResource($socialAssitanceRecipient), 200);
        } catch (\Exception $e) {
            return ResponseHelper::jsonResponse(false, $e->getMessage(), null, 500);
        }
    }
