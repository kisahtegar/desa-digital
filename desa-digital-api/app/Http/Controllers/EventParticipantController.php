...
class EventParticipantController extends Controller implements HasMiddleware
{
    ...
    {
        return [
            new Middleware(PermissionMiddleware::using(permission: ['event-participant-list|event-participant-create|event-participant-edit|event-participant-delete']), only: ['index', 'getAllPaginated', 'show']),
            new Middleware(PermissionMiddleware::using(permission: ['event-participant-create']), only: ['store']),
            new Middleware(PermissionMiddleware::using(permission: ['event-participant-edit']), only: ['update']),
            new Middleware(PermissionMiddleware::using(permission: ['event-participant-delete']), only: ['destroy']),
        ];
    }
    ...
    public function show(string $id)
    {
        try {
            ...
            return ResponseHelper::jsonResponse(true, 'Detail Pendaftar Event Berhasil Ditemukan', new EventParticipantResource($eventParticipant), 200);
        } catch (\Exception $e) {
            return ResponseHelper::jsonResponse(false, $e->getMessage(), null, 500);
        }
    }
